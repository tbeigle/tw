...
//	}
	...
	public static function updatePostParent( $postId, $parentId ) {
		...
		if ( isset( $post ) && $post != null && isset( $parentPost ) && $parentPost != null ) {
			...
			throw new \Exception( 'The post doesn\'t exist: ' . $new_post_id );
		}
	}
	...
	public static function convertCloverDate( $dateToConvert, $format="Y-m-d H:i:s" ){
		
		if ( !$dateToConvert ) {
			return "";
		}

		$timeZone = Settings\WcRegistry::instance()->getCloverMerchant()->getTimezone();
		if ( ! $timeZone ){
			throw new \Exception('Clover Time Zone not configured' );
		}
		...
		$dtNow->setTimestamp($dateToConvert/1000);

		return $dtNow->format($format);
	...
	public static function convertPriceForClover( $price ){
		return $price * 100;
	}
	
	public static function getCloverInfo ( $postId ) {

		$cloverID = get_post_meta( $postId, \Wooclover\WooApi\Config::CloverIdMetaKey, true );
		$lastSyncDate = get_post_meta( $postId, \Wooclover\WooApi\Config::LastSyncDate, true );

		$result = new \stdClass();
		$result->synched = $cloverID ? true : false;
		$result->cloverId = $cloverID ? $cloverID : 'Not sync';
		$result->lastSyncDate = $lastSyncDate ? date( "l jS F Y H:i:s", strtotime( $lastSyncDate ) )  : 'Not sync';

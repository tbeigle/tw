...
class WC_Catalog_Restrictions_Filters {
	...
	public function on_loop_add_to_cart_link($markup, $product){
		...
		if ( !$this->user_can_purchase( $product ) ) {
			...
			return apply_filters( 'catalog_visibility_alternate_add_to_cart_link', sprintf( '<a href="%s" data-product_id="%s" class="button product_type_%s">%s</a>', $link, $product->id, $product->product_type, $label ) );
		}else {
			return $markup;
		}
	}
